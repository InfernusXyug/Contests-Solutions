...
bool bsearch(ll mid) {
    ll start, end;
    //case1
    start = max ({0LL, x1 - mid + 1});
    end = min ({n, x1 + mid - 1,x2 - mid});
    if(start <= end) return true;
    //case2
    start = max ({0LL, x2 + mid, x1 - mid + 1});
    end = min ({n, x1 + mid - 1});
    if(start <= end) return true;
    //case3
    start = max ({0LL, y-mid+1});
    end = min ({n, y + mid - 1, y2 - mid});
    if(start <= end) return true;
    //case4
    start = max ({0LL, y2 + mid, y - mid + 1});
    end = min ({n, y + mid - 1});
    if(start <= end) return true;
    //lwda lassan
    return false;
